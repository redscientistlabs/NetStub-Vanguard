...
void ProcessManager::GetProcessMaps()
{
	...
	uint32_t minaddr = (uint32_t)system_info->lpMinimumApplicationAddress;
	uint32_t maxaddr = (uint32_t)system_info->lpMaximumApplicationAddress;
...
int RPC_HandleRead(int fd, struct RPC_CMD_HDR_PROC_MEM_ACCESS* read_info) {
	...
	int r = 0;
	uint32_t length = read_info->length;
	uint32_t left = length;
	uint32_t offset = 0;
...
int RPC_HandleWrite(int fd, struct RPC_CMD_HDR_PROC_MEM_ACCESS* write_info) {
	uint8_t* data = nullptr;
	int r = 0;
	uint32_t length = write_info->length;
...
int HandleRPCCommand(int fd, struct RPC_PACKET* packet) {
	...
	case RPC_CMD_FLUSHINSN: {
		RPC_CMD_HDR_PROC_MEM_ACCESS* data = (struct RPC_CMD_HDR_PROC_MEM_ACCESS*)packet->data;
		FlushProcessCodeCache(data->handle, data->address, data->length);
		break;
	}
	case RPC_CMD_SETPROTECT: {
		RPC_CMD_HDR_MEM_PROTECTION* data = (struct RPC_CMD_HDR_MEM_PROTECTION*)packet->data;
		DWORD mp;
