namespace NetStub.UI.HexEditor
{
    using System;
    ...
    public partial class HexEditor : Form
    {
        ...
        private bool MakeValue(int dataSize, long address, out int value)
        {
            ...
            try
            {
                if (_domain.ReadOnly && _domain.WholeArray != null)
                {
                    switch (dataSize)
                    {
                        case 1:
                            value = _domain.WholeArray[address];
                            break;
                        case 2:
                            value = BitConverter.ToInt16(new byte[] { _domain.WholeArray[address], _domain.WholeArray[address + 1]}, 0);
                            break;
                        case 4:
                            value = BitConverter.ToInt32(new byte[] { _domain.WholeArray[address], _domain.WholeArray[address + 1], _domain.WholeArray[address + 2], _domain.WholeArray[address + 3] }, 0);
                            break;
                        default:
                            break;
                    }
                ...
                else
                {
                    switch (dataSize)
                    {
                        ...
                        case 2:
                            ...
                            break;
                        case 4:
                            value = BitConverter.ToInt32(_domain.PeekBytes(address, address + 4, !SwapBytes), 0);
                            break;
                    }
                }
            }
        ...
        {
            byte[] value = {firstByte, secondByte};
            if (BigEndian)
            {
                value.FlipBytes();
            }
            _domain.PokeBytes(address, value);
        }
        ...
        {   //Sends text to the VMD Generator and trigger generation
            LocalNetCoreRouter.Route(RTCV.NetCore.Endpoints.UI, RTCV.NetCore.Commands.Remote.GenerateVMDText, new object[] { domain, text }, false);
        }
